    ...
    while True:
        ...
        if grabResult.GrabSucceeded():
            ...
            if clicked == True:
                ...
                vals_retninger = trin_vinkel(theta=theta, r=r, trin=45) #variablen (vals_retninger)  = funktionen med (parameter), hvor det bliver nævnt at 45 er valse retningen 
                ...
                Rundhed = (4 * np.pi * area) / (omkreds ** 2) * 100 # Beregning af rundhed baseret på område og omkreds
                
                # Opdatering af etiketter med afrundede værdier
                ...
                labelRundhed.configure(text=f"Rundhed: {round(Rundhed, 3)}%")# Opdatering af etiket med rundhedsværdi
                ...
                # Beregning af radiale afstande og vinkler med mindre bin-størrelse
                exce = trin_vinkel(theta=theta, r=r, trin=1)
                r_exc = exce[0]
                theta_exc = exce[1]

                # Beregning af forskelle mellem nye radiale afstande og referenceværdier
                delta_r = []
                for i, a in zip(r_exc, r_ref):
                    delta_r.append(i - a)

                # Beregning af mikrotrin og positioner baseret på forskelle
                ...
                # Flyt lærredet (detaljer om funktionen mangler)
                move_canv()

                # Nulstil indikatoren for klik, så analysen ikke gentages
                clicked = False


            # Vent på brugerinput ("q" for at afslutte)
...
def polar_plot():
    ...
    # Overskrift baseret på antal emner
    if antal == 1:
    ...
    # Indstil rækkevidde for radiale afstande
    ...
    ax.set_thetagrids(range(0, 360, 45), labels=custom_labels, color="white")
     ...
     # Plots for observerede og referenceværdier
...
def send_data():
    ...
    # Send informationsmeddelelser om datastørrelse og data
...
# Funktion til at tegne en oval på et lærred
...
# Funktion til at flytte elementer på lærredet
def move_canv():
    # Beregning af x- og y-ændringer baseret på forskelle i positioner
    ...
    # Flytning af elementer på lærredet
