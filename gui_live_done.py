...
#Defineret klick funktion globalt maessigt
def click():
    global clicked
    clicked = True
...
def r_vink_excel(file_path, r, angle):
    ...
    if os.path.exists(file_path):
        # hvis filen eksister, aaben den og load informationen ind i den.
...
#definition af  areal beregninger: hvor der bliver gjordt brug af snoerrebaandsformlen
def calculate_area(x, y):
    # Get the number of vertices (assuming x and y have the same length)
    n = len(x)
    # beregniner ved snoerrebaandsformlen formlen
...
def calibrate(): #Definering af fisheye kalibreringsfunktion
    ...
    im5 = cv2.imread(r"C:\Users\Juliu\OneDrive\Skrivebord\MP3\works\chess5.jpg")
    ...
    img_list = [im1, im2, im3, im4, im5, im6, im7, im8, im9, im10] #Billederne indsaettes i en liste
    # criteria = (cv2.TERM_CRITERIA_EPS + cv2.TERM_CRITERIA_MAX_ITER, 30, 0.001)
    objp = np.zeros((6 * 8, 3), np.float32)     #objp bliver defineret som en variabel for en 48x3 matrix
    objp[:, :2] = np.mgrid[0:8, 0:6].T.reshape(-1, 2)   #objp bliver tilpasset et 2D koordinatsystem til at praesentere skakbraettet

    objpoints = []  # 3D point in real world space
    imgpoints = []  # 2D points in image plane

    for image in img_list:  #Dette er et for loop der kalder alle image variabler
        frame = cv2.cvtColor(image, cv2.COLOR_BGR2RGB)  #Denne funktion aendrer farvecoden fra BGR til RGB
        gray_frame = cv2.cvtColor(frame, cv2.COLOR_BGR2GRAY)    #Denne funktion danner en graascalering
        _, binary = cv2.threshold(gray_frame, 100, 255, cv2.THRESH_BINARY)  #Her bliver der tilfoejet en binaer graense til det graascaleret billede
        ret, corners = cv2.findChessboardCorners(binary, (8, 6), None)  #Funktionen finder hjoernerne paa billedet og # definerer et (8,6) skakbraet moenster til varibelen corners

        if ret == True: #funktionen tjekker om hjoernerne blev fundet fra sidste funktion
            objpoints.append(objp)  #Varibelen objp bliver vedhaeftet til listen objpoints
            imgpoints.append(corners)   #Varibelen corners bliver vedhaeftet til listen imgpoints
    ...
    print("Calibrated") #Denne funktion printer teksten "Calibrated" naar funktionen over er koert
    calibrated = True   #Variabelen calibrated bliver sat til true naar kameraet er kalibraeret
    cv2.destroyAllWindows() #Denne linje lukker alle aabne OpenCV windows efter kalibraringen


def calibrer():     #Definering af funktionen "calibrer"
    if __name__ == "__main__":      #tjekker om sciptet bliver kaert som hovedprogram
        p4 = th.Thread(target=calibrate)    #fremstilling af et thread objekt med navnet p4 med funktions maalet "calibrate"
        p4.start()      #Starter brugen af threaden


def cam_activate():     #Definering af funktionen "cam_activate"
    if __name__ == "__main__":      #tjekker om sciptet bliver kaert som hovedprogram
        p1 = th.Thread(target=camera)       #fremstilling af et thread objekt med navnet p1 med funktions maalet "camera"
        p1.start()      #Starter brugen af threaden

#Her bliver x_0 til x_315 defineret til deres neutral position
x_0 = 3.815
x_45 = 3.815
x_90 = 3.815
x_135 = 3.815       
...
def matrix_ligning(delta_r_vector):
    global x_0
    global x_90
    global x_180        #De justebare kiler bliver defineret paa en maade at alt koden kan se det globalt: (x_0,x_90,x_180,x_270)
    global x_270
...
def trin_vinkel(theta, r, trin):
    #der bliver lavet denne "vals" som er en liste med vaerdiger repraecenteret med en vinkel med en givet step
    vals = [h for h in range(0, 360, trin)]
    #For hver vaerdig in vals, finder den, den vinkel som er taettes paa den korresponderne distance i dataen (theta and r)
    ...
    for c in vals: #Et loop 
        ...
        theta_or = None #Denne variabel vil blive brugt til at gemme vinkelen med minimeret difference til oenskede vinkel
        ...
        for s, rad in zip(theta, r):
            diff = abs(c - s) #beregner den absolute forskel c= den oenskede vinkel og s=som er den nuvaerende vinkel, hvor der kommer ud med en forskel. (note: positive or negative er lige meget for funktionen)
            if diff < min_diff: #linen tjekker for om differancen er mindre en den nuvaerende mindste diff (min_diff). Hvis ja, saa koere koden videre med statmentsene under
                min_diff = diff #updater den minimale differance = diff til den nuvaerende diff
 ...
 #note: r_or og theta_or finde vinkel fra den input dataen som minimere den absolute diff til den oenskede vinkel "c"
...
#defninieret kamera funktion "det der sker naar naar der trykkes paa knappen paa tk.inter brugerfladen"
def camera():
    global r
    global angle
    global theta
    ...
    while True:
        ...
        if grabResult.GrabSucceeded():
            ...
            resized_image = cv2.resize(image, (546 * 2, 364 * 2)) #skalere frame stoerrelse
            ...
            if calibrated == True:
                undistorted_frame = cv2.undistort(frame, mtx, dist, None, mtx) #ikke forvraengning framet med at bruge kamerats parameter
                gray_frame = cv2.cvtColor(undistorted_frame, cv2.COLOR_BGR2GRAY) #conventere ikke forvraengning framet til en grayscale
                blur = cv2.GaussianBlur(gray_frame, (3, 3), 0) #anvender "Gaussian blur" til grayscale billedet til at reducere stoej. 
                # blur = cv2.GaussianBlur(gray_frame, (5, 5), 0)
                _, binary = cv2.threshold(blur, 100, 255, cv2.THRESH_BINARY) #Anvender framet binaer saa alle pixels under 100 bliver 0(sort) og alle over 100 bliver 255(hvide)
                # Display the frame
                ...
                contours.remove(contours[0]) #Linjen fjerner den foerste kontur, da det er selve rammen af framet "firekant"
                #ydre_contour = contours[0]



                cv2.drawContours(undistorted_frame, contours, -1, (0, 0, 255), 5) #Linjen laver vores konturen som fremkommer som en roed linje
                cv2.circle(undistorted_frame, (2736, 1824), radius=5, color=(0, 0, 255), thickness=-1) #tegner den fede markerete dot i midten af skaermen
                cv2.line(undistorted_frame, (2900, 1824), (5000, 1824), color=(0, 0, 255), thickness=5) #tegner en red line paa det ikke forvraengede frame
                cv2.namedWindow("Undistorted", cv2.WINDOW_NORMAL) # laver en navngivet vindue fane med normale stoerrelse forhold
                cv2.resizeWindow("Undistorted", 819, 546) # resizer stoerrelsen af vinduet
                cv2.imshow('Undistorted', undistorted_frame) # displater den endelige frame i det ikke forvraengede frame i et vindue

            #If statement of calibrated er = not true saa sker dette  
            else:
                #De meste er kopi pase fra foer "paanaer nogle variabler"
            ...
            # hvis trykkede = true, saa sker dette
            if clicked == True:
                #count = count + 1
                ...
                for vin in angle:
                    omvendt = -1 * vin
                    #posetiv
                    if omvendt > 0:   #omvendt er er stoerre end 0
                ...
                sorted_theta_r = sorted(zipped_theta_r, key=lambda x: x[0]) #listen sortede baseret paa vinkel
                ...
                #r er infinity negativ og posetiv
                r_max = float("-inf")
                r_min = float("inf")
                ...
                vals_retninger = trin_vinkel(theta=theta, r=r, trin=45) #variablen (vals_retninger)  = funktionen med (parameter), hvor det bliver naevnt at 45 er valse retningen 
                #vinkler de med det forskellige zoner 
                ...
                omkreds = np.sum(np.sqrt(np.diff(x_list) ** 2 + np.diff(y_list) ** 2)) #beregner variablen = Euclidean som er distancen mellem 2 pointer

                # Numerical integration for area using the trapezoidal rule
                area = calculate_area(x_list, y_list) #variablen er lige med den funktionen med (parameterne)  

                Rundhed = (4 * np.pi * area) / (omkreds ** 2) * 100 # Beregning af rundhed baseret paa omraade og omkreds
                
                # Opdatering af etiketter med afrundede vaerdier
                ...
                labelRundhed.configure(text=f"Rundhed: {round(Rundhed, 3)}%")# Opdatering af etiket med rundhedsvaerdi

                ani_data = [r_0, r_45, r_90, r_135, r_180, r_225, r_270, r_315] # Oprettelse af en liste med afrundede radiale afstande

                # Angivelse af filstier til Excel-filer
                ...
                # Beregning af radiale afstande og vinkler med mindre bin-stoerrelse
                exce = trin_vinkel(theta=theta, r=r, trin=1)
                r_exc = exce[0]
                theta_exc = exce[1]

                # Beregning af forskelle mellem nye radiale afstande og referencevaerdier
                delta_r = []
                for i, a in zip(r_exc, r_ref):
                    delta_r.append(i - a)

                # Beregning af mikrotrin og positioner baseret paa forskelle
                ...
                # Gem radiale afstande og vinkler i Excel-fil
                r_vink_excel(file_path=file_path, r=r_exc, angle=theta_exc)
                # Gem positioner i Excel-fil
                k_excel(file_path=file_path1, x=posi)

                # Flyt laerredet (detaljer om funktionen mangler)
                move_canv()

                # Nulstil indikatoren for klik, saa analysen ikke gentages
                clicked = False


            # Vent paa brugerinput ("q" for at afslutte)
            elif cv2.waitKey(1) == ord("q"):
                break
            
            # Frigiv billedresultatet
            grabResult.Release()
...
def polar_plot():
    ...
    # Indstil raekkevidde for radiale afstande
    ax.set_rlim((int(r_min) - 2), (int(r_max) + 2))

    # Gitterlinjer og labels
    ...
    ax.set_thetagrids(range(0, 360, 45), labels=custom_labels, color="white")
     ...
     # Plots for observerede og referencevaerdier
    ax.plot(np.radians(theta), r, color="red")
    ax.plot(np.radians(theta_ref), r_ref, color="black")

    # Integrer polardiagrammet i et tkinter-vindue
    canvas = FigureCanvasTkAgg(fig, master=frame1)
    canvas.get_tk_widget().grid(row=1, column=1, pady=5, padx=5)

# Funktion til at sende data via socket-forbindelse
def send_data():
    data = rotations_data
    data_dump = json.dumps(data)
    data_size = len(data_dump)
    # Send informationsmeddelelser om datastoerrelse og data
...
# Funktion til justering 
def justering():
    clientsocket.send("justering".encode('utf-8'))

# Funktion til at oprette forbindelse via socket
def con():
    global clientsocket
...
# Funktion til at tegne en oval paa et laerred
...
# Funktion til at flytte elementer paa laerredet
def move_canv():
    # Beregning af x- og y-aendringer baseret paa forskelle i positioner
    ...
    # Flytning af elementer paa laerredet
...
def indstil_canv():
    ...
    #Denne kode skaber et animationsloop som bevaeger variblerne Klie0 og lin0 i smaa steps
    n = 0
    while n < 7:
        n += 1
        canvas.move(kile0, -1, 0)
        canvas.move(lin0, -1, 0)
        sleep(0.2)  #Denne linje skaber et delay paa 0.2 sekunder efter hver bevaegelse

    # Denne kode skaber et animationsloop som bevaeger variblerne Klie90 og lin90 i smaa steps
    n = 0
    while n < 7:
        n += 1
    ...
    # Denne kode skaber et animationsloop som bevaeger variblerne Klie180 og lin180 i smaa steps
    n = 0
    while n < 7:
        n += 1
    ...
    # Denne kode skaber et animationsloop som bevaeger variblerne Klie270 og lin270 i smaa steps
    n = 0
    while n < 7:
        n += 1
...
#Koden under bliver brugt til at skaber en brugergraenseoverflade, defineret under navnet GUI
def gui():
    ...
    #BrugerGraenseOverfladen bliver sat til dark mode
    ctk.set_appearance_mode("dark")
    ctk.set_default_color_theme("dark-blue")

    #BrugerGraenseOverfladen bliver tildelt en pixel stoerrelse paa (1230x550)
    window = ctk.CTk()
    window.geometry("1230x550")

    #Der bliver defineret et icon som bliver resizet til en spcifik stoerrelse.
    ...
    #Dette stykke kode bliver brugt til at skabe 6 frames til at oganisere layoutet i BrugerGraenseOverfladen.
    #frame er til knapperne
    frame = ctk.CTkFrame(master=window, width=300, height=300)
    frame.grid(row=0, column=0, pady=5, padx=5)

    #frame1 er til det polaere plot
    ...
    #frame4 er til runhedssoejlen under det polaere plot
    frame4 = ctk.CTkFrame(master=window)
    frame4.grid(row=1, column=1)

    #frame5 er til vaerktoejets projektion
    frame5 = ctk.CTkFrame(master=window, width=500, height=500)
    frame5.grid(row=0, column=4)

    #Her bliver der skabt en knap funktion som gemmer data, som goer brug af iconnet gemt fra tidligere
    button = ctk.CTkButton(master=frame,
                           ...
                           image=tk_excl,
                           command=click)
    button.grid(row=0, column=0, pady=5,padx=5)  # Denne linje beskriver hvilken raekke og kolonne knappen og teksten bliver sat.

    # Her bliver der skabt en knap funktion som skaber et polaert plot, som goer brug af iconnet gemt fra tidligere
    button1 = ctk.CTkButton(master=frame,
                            text="Polar Plot",
                            font=("Times New Roman", 15),
                            command=polar_plot,
                            fg_color="dimgray")
    button1.grid(row=1, column=0, pady=5, padx=5)   #Denne linje beskriver hvilken raekke og kolonne knappen og teksten bliver sat.

    # Her bliver der skabt en knap funktion som sender micro-steps beregningerne til SBC'en,
    # som goer brug af iconnet gemt fra tidligere
    button2 = ctk.CTkButton(master=frame,
                            ...
                            fg_color="dimgray",
                            command=send_data)
    button2.grid(row=2, column=0, pady=5, padx=5)   #Denne linje beskriver hvilken raekke og kolonne knappen og teksten bliver sat.

    # Her bliver der skabt en knap funktion som sender kommandoen med at justere de microsteps som er beregnet paa motoren,
    # som goer brug af iconnet gemt fra tidligere
    button3 = ctk.CTkButton(master=frame,
                            ...
                            fg_color="dimgray",
                            command=justering)
    button3.grid(row=3, column=0, pady=5, padx=5)   #Denne linje beskriver hvilken raekke og kolonne knappen og teksten bliver sat.
    ...
    button4 = ctk.CTkButton(master=frame,
                            ...
                            fg_color="dimgray",
                            command=calibrer)
    button4.grid(row=4, column=0, pady=5, padx=5)   #Denne linje beskriver hvilken raekke og kolonne knappen og teksten bliver sat.

    # Her bliver der skabt en knap funktion som aktivere kameraet, som goer brug af iconnet gemt fra tidligere
    ...
    button5.grid(row=5, column=0, pady=5, padx=5)   #Denne linje beskriver hvilken raekke og kolonne knappen og teksten bliver sat.

    #Denne kode bliver brugt til at skabe tekst der skal staa paa en kolonne
    ...
    #Denne kode bliver brugt til at indsaette tekst labelerne skabt over ind paa forskellige raekker
    ...
    # Denne kode bliver brugt til at lable radiusen fra forskellige vinker der skal staa paa en kolonne
    ...
    # Denne kode indsaetter radius vaerdierne paa raekkerne i en kolonne
    ...
    #Denne kode definere varibelen canvas til at vaere frame5 og giver den dimensionerne 500x500
    canvas = ctk.CTkCanvas(frame5, width=500, height=500, bg='grey25')
    canvas.pack(padx=10, pady=10)


    #Koden under tegner en oval paa et laerred, Ovalen beskriver vaerktoejet
    ...
    #Koden under skaber 4 rectangler paa placeringerne 0°,90°,180°,270° indenfor en oval.
    # disse rectangler bliver defineret som kiler, og bliver brugt til at vise kilens placering i vaerktoejet.
    ...
    #Denne kode skaber en stiplede linje paa laerredet fra start koordinaterne til slut koordinaterne
    start0 = (391, 220)
    end0 = (391, 280)
    ...
    #Denne kode skaber en cyan farvet stiplede linje paa laerredet fra start koordinaterne til slut koordinaterne.
...
#Koden under skaber threads, som bliver koert samtidigt
if __name__ == "__main__":      #Der bliver tjekket om scriptet bliver koert som hovedprogram
